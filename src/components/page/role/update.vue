...
export default {
  ...
  methods: {
    ...
    saveRelative: function() {
      ...
      var selected = that.selectedResource;
      const params = new URLSearchParams();
      params.append("roleId", that.role.id);
      params.append("resourceIds", that.selectedResource);
      params.append("operateMap",JSON.stringify(that.operate));
      // 保存关联信息
      this.axios
        .post(config.remote_site + "/sso/savePermission", params, {
          headers: { token: token }
        })
    ...
    save: function() {
      ...
      // 保存
      this.axios
        .put(url, role, { headers: { token: token } })
        .then(function(response) {
          var result = response.data;
          if (result.code == 0) {
            that.saveRelative();
