    ...
    <div class="btn-toolbar justify-content-between" role="toolbar">
      ...
      <div class="justify-content-end">
        <form class="form-inline">
          ...
          <label class="sr-only" for="phone-search">所属工作项</label>
          <select v-model="pageInfo.otherParam.workflowItemId" class="form-control mb-2 mr-sm-2" placeholder="请选择">
            <option selected hidden value="">请选择所属工作项...</option>
            <option :selected='pageInfo.otherParam.workflowItemId == item.id'  v-for="item in workflowItems" :key="item.id" :value="item.id">{{item.name}}</option>
          </select>
      ...
      <table class="table table-striped table-sm">
        ...
        <tbody>
          <tr v-for="row in pageInfo.rows" :key="row.id">
            <td>{{row.name}}</td>
            <td>{{row.workflowItemName}}</td>
            <td>{{row.nodeNum}}</td>
            <td>{{row.operateUsersDisplayName}}</td>
            <td>{{row.created}}</td>
            <td>{{row.updated}}</td>
            <td v-if="crOperate">
